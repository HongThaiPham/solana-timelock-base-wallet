...
import {
  ...
  ParaSolanaSigner,
} from '@getpara/solana-signers-v2-integration';
...
export function useParaSigner() {
  ...
  useEffect(() => {
    if (isConnected && rpc && client) {
      try {
        const newSigner = createParaSolanaSigner({
          para: client as unknown as Parameters<
            typeof createParaSolanaSigner
          >[0]['para'],
  ...
  }, [isConnected, rpc, client, paraRpc]);
